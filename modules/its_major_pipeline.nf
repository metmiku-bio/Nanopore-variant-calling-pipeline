process ALIGN_SAMPLE_ITS {
    tag "${sample}"
    publishDir "${params.outdir}/its/alignment/${sample}", mode: 'copy'
...
process CALCULATE_COVERAGE_ITS {
    tag "flagstat: ${sample}"
    publishDir "${params.outdir}/its/flagstat", mode: 'copy'
...
process CONSENSUS_PIPELINE_ITS {
    ...
    publishDir "${params.outdir}/its/consensus", mode: 'copy'
...
process CONCATENATE_HAPLOTYPE_ITS {
    conda "${projectDir}/envs/mafft.yml"

    publishDir "${params.outdir}/its/concatenate", mode: 'copy'
    ...
    output:
    path("concatenate_hap1_its_region.fa"), emit: hap1
    path("concatenate_hap2_its_region.fa"), emit: hap2
    path ("concatenate_hap1_its_region.afa"),emit:hap1_mafft
    path ("concatenate_hap2_its_region.afa"),emit:hap2_mafft
    script:
    """
    cat ${hap1_files.join(' ')} > concatenate_hap1_its_region.fa

    cat ${hap2_files.join(' ')} > concatenate_hap2_its_region.fa

    mafft --auto concatenate_hap1_its_region.fa > concatenate_hap1_its_region.afa
    mafft --auto concatenate_hap2_its_region.fa > concatenate_hap2_its_region.afa
    """
}

process RAXML_NG_TREE_ITS {

    conda "${projectDir}/envs/raxml.yml"

    publishDir "${params.outdir}/its/phylogeny", mode: 'copy'
