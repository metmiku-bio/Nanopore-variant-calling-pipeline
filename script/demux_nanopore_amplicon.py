...
def is_on_edge(hit, read_size, edge_size):
    try:
        ...
        # start or end is '-' or non-numeric
        return False

    if end < edge_size or start > read_size - edge_size:
        return True
    else:
        return False
...
def main(args):
    ...
    id2barcodes = {}
    barcodes2id = {}
    for row in csv.DictReader(open(args.barcodes), delimiter="\t"):
    ...
    for bid in barcode_assignments:
        ...
        sp.call(f"seqkit grep -f {tmp_barcode_id_file} {args.fastq} > {fasta_output} 2> /dev/null",shell=True)
...
parser.add_argument('--max-mismatch',type=int,default=1,help='Sample name (lofreq required)')
parser.add_argument('--edge-size',type=int,default=100,help='Sample name (lofreq required)')
